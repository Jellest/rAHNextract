...
#'@param keep.ahn Deault FALSE. Set to TRUE if you want to keep the downloaded AHN area of the point (9 pixels).
#'@author Jelle Stuurman
#'ahn_point(name, X, Y, LONLAT = FALSE, AHN = "AHN3", dem = "dsm", resolution = 0.5, keep.ahn = TRUE, decimals = 2, interpolate = TRUE)
...
ahn_point <- function(X, Y, name, LONLAT = FALSE, AHN = "AHN3", dem = "dsm", resolution = 0.5, keep.ahn = FALSE, decimals = 2, interpolate = TRUE){
  ...
  my_raster <- download_wcs_raster(wcsUrl = my_url, name = name_trim, AHN = AHN, dem = tolower(dem), resolution = resolution, interpolate = interpolate, keep.ahn = keep.ahn)
